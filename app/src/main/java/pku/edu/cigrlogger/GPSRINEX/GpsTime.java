...
public class GpsTime {
    ...
    public GpsTime(GnssClock gnssClock) {
        ...
        this.nsec = TimeNanos - (FullBiasNanos + BiasNanos);// FullBiasNanos , BiasNanos 是负值。long型数据，高位为1 为负。
        //this.nsec = TimeNanos+Math.abs(FullBiasNanos);//和GEO++ rinex logger 打印出来的时间略有不同。原因是双方数据转换时有一些精度丢失。只是打印时有问题
        // 本质不影响伪距等计算。
        calculateGpsTime();
        /* // 此处代码是仿照GEO++ rinex Logger 处理时间。这样处理之后打印出来的时间是一样的，应该只是打印的问题
        long paramLong1 = Math.abs(TimeNanos)+Math.abs(FullBiasNanos);
        Locale locale = Locale.UK;
        long biassecs = Long.valueOf(paramLong1 / 100L % 10000000L);
        int secs = (int)Math.floor(second);
        second = (double)secs+(double)biassecs/10000000.0;
        */
    }
