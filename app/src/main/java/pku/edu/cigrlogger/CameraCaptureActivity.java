...
import pku.edu.cigrlogger.GPSRINEX.GPSManager;
import pku.edu.cigrlogger.gles.FullFrameRect;
import pku.edu.cigrlogger.gles.Texture2dProgram;
...
public class CameraCaptureActivity extends Activity
    ...
    public static final String TAG = "CIGRLogger";
    ...
    private static BarometerManager mBaroManager;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mBaroManager = new BarometerManager(this);
    ...
    protected void onResume() {
        ...
        mBaroManager.register();
        mgpsManager.register(this);//开辟新进程专门处理GPS
    ...
    public void clickToggleRecording(@SuppressWarnings("unused") View unused) {
        ...
        CheckBox NocamCB = (CheckBox) findViewById(R.id.NoCam);
        CheckBox NoGPSCB = (CheckBox) findViewById(R.id.NoGPS);
        Nocam = NocamCB.isChecked();
        Nogps = NoGPSCB.isChecked();
        if (mRecordingEnabled) {
            String outputDir = renewOutputDir();
                ...
                String inertialFile = outputDir + File.separator + "gyro_accel_magm.csv";
                mImuManager.startRecording(inertialFile);
                String BarometerFile = outputDir + File.separator + "pressure.csv";
                mBaroManager.startRecording(BarometerFile);
                if(!Nogps){
                    mgpsManager.startRecording(outputDir);
                }
            ...
            mBaroManager.stopRecording();
        }
        mGLView.queueEvent(new Runnable() {
            @Override
            public void run() {
                // notify the renderer that we want to change the encoder's state
                mRenderer.changeRecordingState(mRecordingEnabled,Nocam);
            }
        });
        updateControls();
    }
 ...
 */
class CameraSurfaceRenderer implements GLSurfaceView.Renderer { // 这个类会被上面那个类调用
    ...
    public CameraSurfaceRenderer(CameraCaptureActivity.CameraHandler cameraHandler,
                                 TextureMovieEncoder movieEncoder) {// 构造函数，初始化mRecordingEnabled 为false
     ...
     */
    public void changeRecordingState(boolean isRecording,boolean Nocam) {
    ...
    public void onDrawFrame(GL10 unused) {
        ...
        if (mRecordingEnabled&&!mNocamEnabled) {
